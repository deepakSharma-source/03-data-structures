...
function findHappiestDay(journal) {
  ...
  // loop through each journal entry starting from second
  for(let i = 1; i <= journal.length; i++) {
    // if current entry has the higher happiness, update highestEntry
    if(journal[i].happiness > happiestEntry.happiness) {
      happiestEntry = journal[i];
    }
  }
  // return the entry with the highest happienss score
...
function mostFrequentActivity(journal) {
  ...
  journal.forEach((entry) => {
    ...
    entry.events.forEach((event) => {
     // if the activity exist in the object, increment the count
...
function generateJournalReport(journal) {
  ...
  const averageHappiness = calculateAverageHappiness(journal); // resuing function from above
  const mostCommonActivity = mostFrequentActivity(journal);// resuing function from above 
